...
const SearchScreen: React.FC<SearchScreenProps> = ({ onSelectResult }) => {
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      try {
        ...
        // In a real app, we'd use a better search index.
        // For now, we'll fetch a larger set and filter client-side for better UX
        ...
        if (activeFilter === 'all' || activeFilter === 'users') {
          ...
          allResults = [...allResults, ...filteredUsers.slice(0, 10)];
        }
        ...
        if (activeFilter === 'all' || activeFilter === 'groups') {
          ...
          allResults = [...allResults, ...filteredGroups.slice(0, 10)];
        }
        ...
        if (activeFilter === 'all' || activeFilter === 'channels') {
          ...
          allResults = [...allResults, ...filteredChannels.slice(0, 10)];
        }
  ...
  const trending = [
    { id: '1', name: 'Zyno Official', members: '1.2M', type: 'channel', icon: '🚀' },
    { id: '2', name: 'Tech News', members: '450K', type: 'channel', icon: '💻' },
    { id: '3', name: 'Global Chat', members: '890K', type: 'group', icon: '🌍' },
    { id: '4', name: 'AI Assistant', members: '2.5M', type: 'bot', icon: '🤖' },
  ];
      ...
      <div className="p-6 bg-white border-b border-gray-100 space-y-6">
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-gray-100 border-none rounded-2xl py-4 pl-12 pr-4 text-sm font-bold text-charcoal focus:ring-2 focus:ring-plum-500/20 focus:bg-white transition-all placeholder:text-gray-400"
          />
        </div>
        ...
        {searchQuery ? (
              ...
              {results.map((result) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-gray-100 flex items-center justify-center text-xl shadow-inner group-hover:bg-plum-50 group-hover:text-plum-600 transition-colors">
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                          isMember(result) 
                            ...
                            : 'bg-plum-50 text-plum-700 hover:bg-plum-700 hover:text-white'
                        }`}
                      >
                        {joiningId === result.id ? (
                          <div className="w-3 h-3 border-2 border-plum-700/30 border-t-plum-700 rounded-full animate-spin" />
                        ) : isMember(result) ? (
          ...
          <div className="p-6 space-y-8">
                ...
                {trending.map((item) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-lg shadow-inner group-hover:bg-plum-50 transition-colors">
                        {item.icon}
                      </div>
                      <div className="bg-plum-50 text-plum-600 text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest">
                        {item.type}
                      </div>
                    </div>
                    <div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-br from-plum-700 to-plum-900 p-6 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <button className="w-full bg-white text-plum-700 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg hover:scale-[1.02] active:scale-95 transition-all">
                    Upgrade Now
                  </button>
                </div>
              </div>
